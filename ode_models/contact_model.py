...
from ode_models.symeig import symeig_proj, symeig1, symeig2
...
class ContactModel(nn.Module):
    ...
    def add_impulse(self, xv, mus, cors, Minv, cf3):
        bs = xv.shape[0]
        n, d = self.n, self.d
        ...
        if cf3:
            is_cld_ij[:,-1,:] = torch.zeros(is_cld_ij[:,-1,:].shape, dtype=torch.bool).to(is_cld_ij)
            is_cld_ij[:, :, -1] = torch.zeros(is_cld_ij[:,:,-1].shape, dtype=torch.bool).to(is_cld_ij)
            is_cld_bdry[:,-1] = torch.zeros(is_cld_bdry[:,-1].shape, dtype=torch.bool).to(is_cld_bdry)
            is_cld = torch.logical_or(is_cld_ij.sum([1, 2]) > 0, is_cld_bdry.sum([1, 2]) > 0)
    ...
    def get_dv_w_J_e(self, bs_idx, v, Minv, Jac, Jac_v, v_star, mu, cor, DPhi):
        ...
        e, V = symeig1(Q)
        if not torch.allclose(e, (e>0.5).to(dtype=e.dtype), atol=1e-2):
        ...
        with torch.no_grad():
            if n_cld > n: # Jac is a tall matrix
                ...
                try:
                    ...
                    v_star_euc = torch.linalg.solve(Jac_full_rank.t() @ Jac_full_rank, v_star_euc)
